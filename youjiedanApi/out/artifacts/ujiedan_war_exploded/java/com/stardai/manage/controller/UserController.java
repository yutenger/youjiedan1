...
public class UserController extends BaseController {
		...
		try {
			synchronized (POOL.intern(mobileNumber)) {
				...
				if (loginedUser != null) {
					...
					String cities = loginedUser.getCities();
    ...
    public ResponseModel smsCode(@RequestBody RequestSmsCode smsCodeRequest, BindingResult bindingResult) {
        ...
        try {
            synchronized (POOL.intern(smsCodeRequest.getMobileNumber())) {
                ...
                //1 是注册用户 2 是找回密码 3 验证码登陆 4 更换手机，使用旧手机号发送验证码时 5 更换手机，使用新手机号发送验证码时
                ...
                if (type == 1) {//注册用户
                    ...
                    if (!existence) {
                        return ResponseModel.fail("请输入正确的手机号码");
                    }
	...
	public ResponseModel faceRecognize(@RequestBody RequestPersonVerifyInfo requestPersonVerifyInfo,
		...
		if(type == 0){
			...
			if (result == 1) {
				...
				return ResponseModel.fail("人脸验证失败！");
			}else{
				return ResponseModel.fail("网络异常，认证失败！");
			}
	...
	public ResponseModel approveCompany(@RequestBody RequestCompany company, HttpServletRequest request,
		...
		if (company.getCompanyAddress() == null || company.getCompanyAddress().isEmpty()
				...
				|| company.getCompanyName().isEmpty() || company.getCompanyType() == null
				|| company.getCompanyType().isEmpty()) {
			return ResponseModel.fail("信息填写不完整");
		}
		// company.setCompanyCity(loanUserService.unifyLocations(company.getCompanyCity()));
		String images = null;
		String name = userService.queryNameByUserId(company.getUserId());
		if (name == null) {
			return ResponseModel.fail("请先实名认证");
		} else {
			...
			// 查询企业认证是第一次认证还是修改
			Integer status = userService.queryApproveCompanyByUserId(company.getUserId());
			if (status == 0) {
				images = userService.GenerateImage(company, request, name);
				boolean approvePerson = userService.addApproveCompany(company, images);
				if (approvePerson) {
					payMoneyService.addMessage(company.getUserId(), 1, "审核中",
	...
	public ResponseModel queryApproveCompanyV2(@RequestParam("userId") String userId) {
		Integer result = userService.queryApproveCompany(userId);
		if (result == null) {
			ResponseApprove responseApprove = new ResponseApprove(0, 0, 0);
			return ResponseModel.success(responseApprove);
		} else {
			Integer hasApproved = userService.queryHasApproved(userId);
			ResponseApprove responseApprove = new ResponseApprove(0, result, hasApproved);
			return ResponseModel.success(responseApprove);
		}
