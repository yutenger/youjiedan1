...
public class FaceRecognition {
	...
	public static JsonNode personverify(RequestPersonVerifyInfo requestPersonVerifyInfo) {
		...
		try {
			...
			jsonNode = MAPPER.readTree(jsonTree);
			/*int errorCode = Integer.parseInt(jsonNode.get("error_code").toString());
			if(errorCode == 0 ){
			...
			}else if(errorCode == Constants.image_url_download_fail){
				result = true;
				//userMapper.addFailureReason();
			}else if(errorCode == Constants.police_picture_not_exist){
				result = true;

			}else if(errorCode == Constants.police_picture_is_none){
				result = true;
