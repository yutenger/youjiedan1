...
public class ResponseGetCoupon implements Comparable<ResponseGetCoupon>{
	
	private String couponCode;//优惠券码，用来标识是哪种优惠券
	
	private int couponAmount; // 优惠券面额

	private int couponDiscount; // 优惠券折扣  仅当优惠券是打折券的时候使用

	private int couponFullreduction; // 优惠券是满减券的时候使用，该值是需要满足的金额
	...
	private String cities; //可以领优惠券的城市

	private int sortSign; //排序标志
	...
	public String toString() {
		return "ResponseGetCoupon{" +
				"couponCode='" + couponCode + '\'' +
				", couponAmount=" + couponAmount +
				", couponDiscount=" + couponDiscount +
				", couponFullreduction=" + couponFullreduction +
				", couponTypeName='" + couponTypeName + '\'' +
				", couponFormName='" + couponFormName + '\'' +
				", couponGetStatus=" + couponGetStatus +
				", beginTime='" + beginTime + '\'' +
				", leftPercent=" + leftPercent +
				", couponInitialCount=" + couponInitialCount +
	...
	@Override
	public int compareTo(ResponseGetCoupon o) {
        if(this.sortSign>o.getSortSign()){
             return 1;
		}
		if(this.sortSign<o.getSortSign()){
			return -1;
		}
		if(o.getCouponType()==1) {
			if (this.couponAmount < o.couponAmount) {
				return -1;
			}
			if (this.couponAmount > o.couponAmount) {
				return 1;
