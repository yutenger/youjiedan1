...
public class CreditController extends BaseController {
	...
	protected static final Logger LOG = LoggerFactory.getLogger(CreditController.class);
	...
	public ResponseModel addCreditForNewEditionAndroid(@RequestBody RequestAddCreditByNewEdition versionInfo,
		...
		if(iosVersion == null){
			...
			iosVersion = iosVersion.substring(1,iosVersion.length());
		}
		...
		if (versionCode.equals(version.getVersionCode())) { // 用户当前是最新版本
			...
			if (isGetCredit == null) {// 没有获取过积分
				...
				//为用户送充值满减券 各3张
//				couponService.sendRechargeCouponForNewEditionUser(userId);
