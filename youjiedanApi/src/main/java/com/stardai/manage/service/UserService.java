...
public class UserService {
    ...
    public ResponseUserInfo approvePerson(RequestPersonVerifyInfo person) {
        ...
        //用户个人认证时人脸识别
        JsonNode jsonNode = FaceRecognition.personverify(person);
        if (jsonNode == null) {
            //token 获取失败
            userInfoResult.setApprovePerson(0);
            return userInfoResult;
        } else {

            errorCode = Integer.parseInt(jsonNode.get("error_code").toString());
            if (errorCode == 0) {
                if ((new Double(Double.parseDouble(jsonNode.get("result").get("score").toString()))).intValue() >= 70) {
                    flag = 1;
                }
            } else {
                if (errorCode == Constants.image_url_download_fail || errorCode == Constants.police_picture_not_exist
                        || errorCode == Constants.police_picture_not_exist) {
                    flag = 1;
                }
                LOG.info("个人认证人脸识别：" + person.getUserId() + "：" + jsonNode.toString());
            }
        }
        ...
        if (flag == 0) {
            ...
            if (result != 0) {
                ...
                Integer isExist = userMapper.getUserExistFromExtended(person.getUserId());
                if (isExist != null && isExist >= 1) {
                    //更新扩展表里的图片
                    userMapper.updateUserApproveImage(person);
                } else {
                    // 向用户信息扩展表中添加用户图片
                    this.userMapper.insertUserApproveImage(person);
                }
    ...
    // 根据userId查询此人有没有认证
    public Integer queryApproveByUserIdNew(String userId) {
        ResponseApprove result = userMapper.queryApproveByUserId(userId);
        if (result.getApprovePerson() == 0 || result.getApproveCompany() == 0) {
            return 4;//未认证
        }
        if (result.getApproveCompany() == 1 && result.getApprovePerson() == 1) {
            return 1; //认证成功
        }
        if (result.getApproveCompany() == 2) {
            return 2;//认证中
        }
        if (result.getApproveCompany() == 3) {
            return 3;//认证失败
        }
    ...
    // 根据UserID查询用户认证状态
    public ResponseApprove queryApproveStatus(String userId) {
        return userMapper.queryApproveStatus(userId);
    }
    ...
    public int getfaceRecognizeByChangePhone(RequestPersonVerifyInfo requestPersonVerifyInfo) {
        ...
        if (userInfo != null) {
            ...
            int errorCode = 0;
            int flag = 0;
            JsonNode jsonNode = FaceRecognition.personverify(requestPersonVerifyInfo);
            if (jsonNode == null) {
                //token 获取失败
                return 3;
            } else {
                ...
                if (errorCode == 0) {
                    ...
                    LOG.info("更换手机号人脸识别：" + userId + "：" + jsonNode.toString());
                }



                /*errorCode = Integer.parseInt(jsonNode.get("error_code").toString());
                if(errorCode == 0 && (new Double(Double.parseDouble(jsonNode.get("result").get("score").toString()))).intValue() >= 70){
                    flag = 1;
                }else {
                    LOG.info("更换手机号人脸识别："+userId + "：" +jsonNode.toString());
                }*/
    ...
    //查询此人之前有没有企业认证过
    public String queryIsFirstSubmitApproveCompany(String userId) {
        return userMapper.queryIsFirstSubmitApproveCompany(userId);
    }

    //添加用户登陆的ip或者定位城市
    public Integer addUserLoginLocation(RequestUserLoginLocation rull) {
        return userMapper.addUserLoginLocation(rull);
    }

    //查看用户之前是否添加过相同的ip
    public Integer getIsAddSameIp(RequestUserLoginLocation rull) {
        return userMapper.getIsAddSameIp(rull);
    }

    //更新用户ip和所在位置
    public Integer updateUserLoginLocation(RequestUserLoginLocation rull) {
        return userMapper.updateUserLoginLocation(rull);
    }

    public Integer queryApprovePerson(String userId) {
        return userMapper.queryApprovePerson(userId);
    }
}
